...
def main(argv=None):
	...
	# source_csv = 'medicare_data_1212.csv'
	#source_path = os.path.join('input', 'csv', 'medicare_data_1212.csv')
	source_path = 'medicare_data_1212.csv'
	this_encoding = find_encoding(source_path)
	source_data_frame = pd.read_csv(source_path, encoding = this_encoding)


	# source_data_frame = read_csv(source_csv)
	source_data_frame_trimmed = trim_columns(source_data_frame)
	source_csv_output = 'medicare_data_output.csv'
	write_series_to_csv(source_data_frame_trimmed, source_csv_output, ',', False)
	logging.info(msg[1].format(os.path.abspath(source_csv_output)))
	## CHNOTE ADDED STEP TO WRITE NEW VERSION OF CSV 


	#unsd_data_frame = read_csv(unsd_csv) # may need to add ',' as a param
	logging.info(msg[0].format(os.path.abspath(source_csv_output)))

	# Write drg_desc to a .csv file.
	drg_desc = extract_filtered_series(source_data_frame_trimmed, 'drg_desc')
	...
	old_provider_id = extract_filtered_series(source_data_frame_trimmed, 'old_provider_id')
	...
	city_state_name = extract_filtered_series(source_data_frame_trimmed, 'city_state_name')
	...
	street = extract_filtered_series(source_data_frame_trimmed, 'street')
	...
	referral_region_desc = extract_filtered_series(source_data_frame_trimmed, 'referral_region_desc')
