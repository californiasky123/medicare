...
-- This should populate my referral_region table with my unique values I pulled from the t_medicare_1 table. 

INSERT IGNORE INTO referral_region(referral_region_desc) SELECT DISTINCT referral_region_desc FROM t_medicare_1;

INSERT IGNORE INTO city_state(city_state_name) SELECT DISTINCT city_state_name FROM t_medicare_1;

INSERT IGNORE INTO drg(drg_desc) SELECT DISTINCT drg_desc FROM t_medicare_1;

INSERT IGNORE INTO provider_temp(old_provider_id) SELECT DISTINCT old_provider_id FROM t_medicare_1;

INSERT IGNORE INTO address_temp(street) SELECT DISTINCT street FROM t_medicare_1;
