...
-- -- Insert info from temp table
-- --#chnote -- note that this is using address_temp not address 
-- --#chnote -- note that this is using provider_temp not provider
...
-- #chnote OLD ATTEMPT THAT IS NO LONGER NEEDED based on convo with Anthony 
-- INSERT IGNORE INTO address_temp (zip, city_state_id)
--   SELECT cs.city_state_id,
--          tm.zip
--     FROM city_state cs 
--         LEFT JOIN t_medicare_1 tm ON at.city_state_id = tm.city_state_id
--     FROM t_medicare_1 tm
--         LEFT JOIN city_state 
--         -- WHERE IFNULL(at.city_state_id, 0) = IFNULL(cs.city_state_id, 0)
--    ORDER BY at.street;
...
--        at.zip,
--        at.city_state_id
--   FROM address_temp at
--       LEFT JOIN city_state cs ON at.city_state_id = cs.city_state_id
--       -- WHERE IFNULL(at.city_state_id, 0) = IFNULL(cs.city_state_id, 0)
--  ORDER BY at.street;



-- -- Create non-temp version of provider rtable 
...
--     FOREIGN KEY (address_id) REFERENCES address_temp(address_id) ON DELETE RESTRICT ON UPDATE CASCADE
--   )
-- -- #CHNOTE -- may need to change address_temp above to address 
-- ENGINE=InnoDB
-- CHARACTER SET utf8mb4
-- COLLATE utf8mb4_0900_ai_ci;
...
-- SELECT pt.old_provider_id,
--        pt.provider_name,
--        pt.referral_region_id,
--        pt.address_id
--   FROM provider_temp pt
--         LEFT JOIN referral_region rr ON pt.referral_region_id = rr.referral_region_id
--         LEFT JOIN address_temp ad ON pt.address_id = ad.address_id
--      --  WHERE IFNULL(pt.referral_region_id, 0) = IFNULL(rr.referral_region_id, 0)
--      -- AND IFNULL(pt.address_id, 0) = IFNULL(ad.address_id, 0)
--  ORDER BY pt.old_provider_id;
